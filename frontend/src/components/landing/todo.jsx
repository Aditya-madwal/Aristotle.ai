...
import { Plus, Circle, CheckCircle2, ArrowLeftFromLine, ArrowRightFromLine, Tag, X } from 'lucide-react';
...
const TaskSidebar = () => {
    ...
    return (
        <div
            className={`fixed top-0 right-0 h-screen bg-white border-l transition-all duration-300 ${isOpen ? 'w-80' : 'w-12'}`}
                ...
                <div className="h-full flex flex-col pt-16">
                    ...
                    <div className="flex-1 overflow-y-auto px-6">
                        {tasks.map((task) => (
                            <div
                                key={task.uid}
                                className={`mb-4 p-4 rounded-lg ${task.status ? 'bg-gray-50' : 'bg-yellow-50'}`}
                            >
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p className={`text-sm ${task.status ? 'line-through text-gray-500' : 'text-gray-700'}`}>
                                            {task.description}
                                        </p>
                                        {task.label && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                <span className={`px-2 py-1 rounded-full text-xs ${labelOptions.find(l => l.name === task.label)?.color}`}>
                                                    {task.label}
                                                </span>
                                            </div>
                                        )}
                    ...
                    <div className="p-6 flex flex-col gap-3">
                        {selectedLabel && (
                            <div className="flex flex-wrap gap-2 mb-2">
                                {(() => {
                                    const labelOption = labelOptions.find(l => l.name === selectedLabel);
                                    return (
                                        <span
                                            key={selectedLabel}
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {labelOptions.map((label) => (
                                            <button
                                                ...
                                                className={`${label.color} px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors
                                                    ${selectedLabel === label.name ? 'ring-2 ring-offset-1' : 'opacity-80 hover:opacity-100'}`}
                                            >
                                                {selectedLabel === label.name && <CheckCircle2 size={14} />}
                                                {label.name}
                                            </button>
                                        ))}
                                    </div>
